...
\begin{proof}[Proof of the packing threshold (Lemma~5 in the paper)]
...
generalize Proposition~2: let level-$0$ stations occupy measure
...
\begin{proof}[Proof of Theorem~1 (8-approximation)]
...
valid by Lemma~2). \emph{Density scaling:} for $\rho \le 1$,
...
Lemma~5 places everything; each station transmits only $d_i$ inside its
...
\begin{proof}[Proof of Corollary~1 (candidate-argmin anchor)]
...
\begin{proof}[Proof of Theorem~2 (unconditional)]
By Lemma~2 the relaxation may include $T_i \ge \dmax$. Set
...
so Lemma~5 packs. Cost $\le 2 \cdot 6\, J^{*\prime} \le 12\,\mathrm{OPT}$.
