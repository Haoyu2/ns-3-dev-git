...
#include "my-point-to-point-dumbbell.h"
...
NS_LOG_COMPONENT_DEFINE("MyPointToPointDumbbellHelper");

MyPointToPointDumbbellHelper::MyPointToPointDumbbellHelper(uint32_t nLeftLeaf,
                                                           ...
                                                           )
{
    ...
    // Add the link connecting routers
    m_routerDevices = bottleneckHelper.Install(m_routers);
    m_neckQueueContainer.Add(neckQueue.Install(m_routerDevices));
    // Add the left side links
    for (uint32_t i = 0; i < nLeftLeaf; ++i)
    {
        NetDeviceContainer c = leftHelper.Install(m_routers.Get(0), m_leftLeaf.Get(i));
...
MyPointToPointDumbbellHelper::~MyPointToPointDumbbellHelper()
{
}

QueueDiscContainer
MyPointToPointDumbbellHelper::GetNeckQueueContainer() const
{ // Get the left side bottleneck router
    return m_neckQueueContainer;
}

Ptr<Node>
MyPointToPointDumbbellHelper::GetLeft() const
{ // Get the left side bottleneck router
    return m_routers.Get(0);
}

Ptr<Node>
MyPointToPointDumbbellHelper::GetLeft(uint32_t i) const
{ // Get the i'th left side leaf
    return m_leftLeaf.Get(i);
}

Ptr<Node>
MyPointToPointDumbbellHelper::GetRight() const
{ // Get the right side bottleneck router
    return m_routers.Get(1);
}

Ptr<Node>
MyPointToPointDumbbellHelper::GetRight(uint32_t i) const
{ // Get the i'th right side leaf
    return m_rightLeaf.Get(i);
}

Ipv4Address
MyPointToPointDumbbellHelper::GetLeftIpv4Address(uint32_t i) const
{
    return m_leftLeafInterfaces.GetAddress(i);
}

Ipv4Address
MyPointToPointDumbbellHelper::GetRightIpv4Address(uint32_t i) const
{
    return m_rightLeafInterfaces.GetAddress(i);
}

Ipv6Address
MyPointToPointDumbbellHelper::GetLeftIpv6Address(uint32_t i) const
{
    return m_leftLeafInterfaces6.GetAddress(i, 1);
}

Ipv6Address
MyPointToPointDumbbellHelper::GetRightIpv6Address(uint32_t i) const
{
    return m_rightLeafInterfaces6.GetAddress(i, 1);
}

uint32_t
MyPointToPointDumbbellHelper::LeftCount() const
{ // Number of left side nodes
    return m_leftLeaf.GetN();
}

uint32_t
MyPointToPointDumbbellHelper::RightCount() const
{ // Number of right side nodes
    return m_rightLeaf.GetN();
}

void
MyPointToPointDumbbellHelper::InstallStack(InternetStackHelper stack)
{
...
void
MyPointToPointDumbbellHelper::AssignIpv4Addresses(Ipv4AddressHelper leftIp,
...
void
MyPointToPointDumbbellHelper::AssignIpv6Addresses(Ipv6Address addrBase, Ipv6Prefix prefix)
{
    // Assign the router network
...
void
MyPointToPointDumbbellHelper::BoundingBox(double ulx,
